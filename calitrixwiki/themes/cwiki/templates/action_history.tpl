...
{foreach from="$versions" key="page_version" item="log"}
 <tr>
  ...
  <td nowrap="">
   (<a href="{wikiurl page="`$pageName`" v="`$log.log_page_version`"}" >{$lang.history_view}</a> | 
   <a href="{wikiurl page="`$pageName`" action="edit" v="`$log.log_page_version`"}">{$lang.history_restore}</a>)
  </td>
  <td>{if $log.user_name != ''}<a href="{wikiurl page="`$cfg.users_namespace`:`$log.user_name`"}">{$log.user_name}</a>{/if}&nbsp;{if $log.log_summary != ''}<span class="light-grey">({$log.log_summary})</span>{/if}</td>
 </tr>
{/foreach}
</table>
{include file="page_links.tpl"}
