...
{section name="idx" loop="$changes"}
 <tr>
  ...
  <td>{if $changes[idx].user_name != ''}<a href="{wikiurl page="`$cfg.users_namespace`:`$changes[idx].user_name`"}">{$changes[idx].user_name}</a>{/if}&nbsp;{if $changes[idx].log_summary != ''}<span class="summary">({$changes[idx].log_summary})</span>{/if}</td>
 </tr>
{/section}
</table>
